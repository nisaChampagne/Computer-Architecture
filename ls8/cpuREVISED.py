...
class CPU:
    ...
    def run(self):
        ...
        while running:
            ...
            self.ir = self.ram_read(self.program_counter)
            operand_a = self.ram_read(self.program_counter+1)
            operand_b = self.ram_read(self.program_counter+2)

            # For some reason, loop would try to go on after HLT instruction it would go to instruction_register = 1 and throw exception message
            if self.ir == HLT:
                running = False
                break
            ...
            try:
                ...
                print(f"Bad instruction register: {self.ir}")
                sys.exit(1)